...
const HomePageAppBar = (props: HomePageAppBarProps) => {
  const t = useTranslations();

  return (
    <header {...props} className="sticky top-4">
          ...
          <div className="flex items-center gap-1">
            {links.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="px-4 py-3 text-base leading-6 text-gray-250 hover:bg-gray-150/10 hover:text-gray-400 rounded-4xl"
              >
                {t(link.labelKey)}
              </Link>
            ))}
